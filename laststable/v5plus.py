...
def load_config(path: str) -> dict:
    ...
    cfg.setdefault("refresh_sec", 5)

    # NEW: metadata prefetch
    cfg.setdefault("prefetch_metadata", True)
    cfg.setdefault("prefetch_workers", 1)  # klidne 1-2; vic je zbytecne
...
def estimate_bytes_from_info(info: dict) -> int:
    """
    Snazi se vytahnout velikost z metadata (bez downloadu).
    Vraci bytes nebo 0.
    """
...
class GrabberDaemon:
    def __init__(self, cfg: dict):
        ...
        # NEW: metadata prefetch queue
        self.meta_q: queue.Queue[str] = queue.Queue()

        # column widths
        self.W_ID = 17          # o chlup mensi
        self.W_TOTAL = 9        # mensi sloupec = mensi mezera vizualne
    ...
    def _render(self, first: bool = False):
        ...
        # mensi separator: jen 1 mezera mezi sloupci
    ...
    def meta_loop(self):
        while not self.stop_event.is_set():
            ...
            try:
                ...
                with self.lock:
                    # nastav jen kdyz jeste nema total (neprepisuj real total z download hooku)
                ...
                # fail silently; nechame total jako ?
                with self.lock:
                    j = self.jobs.get(job_id)
                    if j:
                        j.meta_done = True
