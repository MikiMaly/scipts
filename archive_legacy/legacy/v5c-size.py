...
class GrabberDaemon:
    ...
    def _render(self):
        ...
        print(f"Workers: {self.cfg['max_workers']} | Frags: {fr} | Queue: {qsize} | queued={queued} active={active} done={done} fail={fail}\n")
        ...
        for j in jobs_sorted[:25]:
            ...
            # TOTAL size (magenta), show ? if unknown
            total_txt = fmt_size(j.total)
            total_part = c(total_txt.rjust(8), MAGENTA)
            ...
            if j.status == "downloading":
                ...
                print(f"{jid} {total_part} {bar} {pct} {sp} {eta_part} {elap_part}")

            elif j.status == "queued":
                # queued: show size unknown
                print(f"{jid} {total_part} {c('[QUEUED]', DIM)} {j.url}")

            elif j.status == "done":
                # prefer final file size if available
                final_bytes = 0
                if j.final_path:
                    try:
                        ...
                        final_bytes = 0
                done_total = c(fmt_size(final_bytes) if final_bytes else fmt_size(j.total), MAGENTA)
                ...
                print(f"{jid} {done_total.rjust(8)} {c('[DONE]', DIM)} in {elap_txt} -> {c(saved, DIM)}")

            elif j.status == "fail":
                err = (j.error[:140] + "…") if len(j.error) > 140 else j.error
                print(f"{jid} {total_part} {c('[FAIL]', RED)} {err}")
