...
class GrabberDaemon:
    ...
    def _render(self, first: bool = False):
        # make UI resilient: if anything goes wrong, show error instead of dying
        try:
            if first:
                ansi_clear_screen()
            else:
            ...
            # Sort into blocks:
            # QUEUED (FIFO) -> DOWNLOADING -> DONE (newest first) -> FAIL (newest first)
            ...
            def render_row(j: Job):
                ...
                # fallback
                print(f"{jid} {total_col} {c('[UNKNOWN]', DIM)}")
            ...
            # never let UI thread die; show error
            ansi_home()
            ansi_clear_to_end()
