...
# =========================
# v6 - Video Grabber
# UI: header boxed per-row + ENQ aligned using terminal width
...
class GrabberDaemon:
    ...
    def _print_box_row(self, text: str, width: int):
        # "Obalit kazdy radek nahore zvlast"
        print("-" * width)
        print(text)
    ...
    def _render(self, first: bool = False):
        try:
            ...
            # keep a sane minimum for formatting
            if term_w < 90:
                term_w = 90
            ...
            # Header boxed per-row
            self._print_box_row("Video Grabber (paste URL + Enter). Ctrl+C to quit.", term_w)
            self._print_box_row(f"Output: {self.out_dir}", term_w)
            self._print_box_row(
            ...
            self._print_box_row(f"CSV: {csv_info} ({csv_count})", term_w)
            print("-" * term_w)
            print()
            ...
            table_head = f"{col_jid} {col_id} {col_total} STATUS/PROGRESS"
            print(table_head)
            print("-" * min(term_w, len(table_head) + 70))
            ...
            def print_aligned(left_colored: str, right_plain: str):
                # Align right_plain to terminal width, considering ANSI in left_colored
                left_visible_len = len(strip_ansi(left_colored))
                right_len = len(right_plain)
                spaces = max(1, term_w - left_visible_len - right_len)
                print(left_colored + (" " * spaces) + right_plain)
            ...
            def render_row(j: Job):
                ...
                if j.status == "downloading":
                    ...
                    # keep downloading row as original layout (stable), but still fits in terminal
