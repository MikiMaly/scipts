...
# - header order: CSV -> Output -> (blank) -> Workers
# - queue_limit + pending + feeder thread
# - timestamp + jid naming
# - queued -> downloading -> done -> fail ordering
# - queued shows [QUEUED] + ENQ aligned to far right inside box
...
def load_config(path: str) -> dict:
    ...
    # Box width: GUI never wider than this (unless terminal is smaller)
...
def shorten_path_end(path_str: str, max_len: int) -> str:
    """
    Keep the end (filename) visible.
    Example: D:\very\long\...\file.mp4
    """
    if max_len <= 0:
        return ""
    if not path_str:
...
class GrabberDaemon:
    ...
    def _render(self, first: bool = False):
        try:
            ...
            # Box header: CSV -> Output -> blank -> Workers
            ...
            def box_row_with_right(left_colored: str, right_plain: str):
                left_len = len(strip_ansi(left_colored))
                right_len = len(right_plain)
                spaces = max(1, inner - left_len - right_len)
            ...
            def render_job(j: Job):
                ...
                if j.status == "done":
                    ...
                    # shorten path to fit inside box
                    # we print: "<cols> [DONE] in H:MM:SS -> <path>"
                    prefix_plain = f"{jid_txt} {self._pad(j.job_id, self.W_ID)} {self._rpad(done_total_txt, self.W_TOTAL)} [DONE] in {elap_txt} -> "
                    # prefix_plain is uncolored length; but our actual prefix includes colors; still safe because we compute max for path based on plain
                    max_path = max(10, inner - len(prefix_plain))
