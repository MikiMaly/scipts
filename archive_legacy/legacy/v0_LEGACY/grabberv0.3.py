...
class GrabberDaemon:
    ...
    def _render(self):
        ...
        for j in jobs_sorted[:25]:
            ...
            if j.status == "downloading":
                ...
                eta = fmt_mmss(j.eta)
                elap = fmt_mmss((now - j.started_at) if j.started_at else 0)

                print(f"{jid} {bar} {pct} {sp} ETA {eta} ELAP {elap}")

            elif j.status == "queued":
                print(f"{jid} {c('[QUEUED]', DIM)} {j.url}")

            elif j.status == "done":
                elap = fmt_mmss((j.finished_at - j.started_at) if (j.started_at and j.finished_at) else 0)
                saved = j.final_path or "(path unknown)"
                # done line: job_id blue + saved path dim
                print(f"{jid} {c('[DONE]', DIM)} in {elap} -> {c(saved, DIM)}")

            elif j.status == "fail":
                err = (j.error[:140] + "…") if len(j.error) > 140 else j.error
                print(f"{jid} {c('[FAIL]', DIM)} {err}")
    ...
    def _build_ydl_opts(self, job: Job) -> dict:
        # base filename = timestamp; yt-dlp picks ext, then we force mp4 via remux
        ...
        opts = {
            ...
            # ensure single output with audio+video (needs ffmpeg)
            "merge_output_format": "mp4",
            # after download+merge, remux to mp4 (keeps streams, no re-encode)
            "remuxvideo": "mp4",
            ...
            # IMPORTANT: kill yt-dlp own console progress lines
            "quiet": True,
            "no_warnings": True,
            "noprogress": True,
    ...
    def worker_loop(self):
        while not self.stop_event.is_set():
            ...
            try:
                ...
                # if remux succeeded, mp4 should exist; try to prefer it
