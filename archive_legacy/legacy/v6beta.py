...
# =========================
# v6.0-boxed (based on v5.9)
# - fixed-width boxed UI (never wider than ui_width)
# - swap CSV and Workers lines (Workers at bottom)
# - queue_limit + pending + feeder thread
...
def load_config(path: str) -> dict:
    ...
    # NEW: boxed UI width (max width). GUI never wider than this.
    # If terminal is narrower, we will use terminal width instead.
...
class Job:
    jid: int
    job_id: str  # timestamp + jid (base filename)
    url: str
...
class GrabberDaemon:
    def __init__(self, cfg: dict):
        ...
        # you said you already tuned this: 64 (from 58)
        self.QUEUED_MIDDLE_W = 64
        self.RIGHT_LABEL_W = 12  # ENQ 00:00:00 / ELAP 0:00:00 (same width concept)
    ...
    def _ui_width(self) -> int:
        ...
        # minimum so the box is not broken
        return max(80, w)
    ...
    def _box_line(self, content: str, w: int) -> str:
        inner = w - 4  # "| " + inner + " |"
        # If content contains ANSI and is too long, fallback to plain truncation
        plain = strip_ansi(content)
        if len(plain) > inner:
            # truncate plain; drop colors to keep box aligned
            plain = plain[:inner]
            content = plain
        # pad based on visible length
        vis = len(strip_ansi(content))
        if vis < inner:
            content = content + (" " * (inner - vis))
        return f"| {content} |"

    def _box_sep(self, w: int) -> str:
        return self._box_border(w)
    ...
    def _render(self, first: bool = False):
        try:
            ...
            inner = w - 4  # content area width

            def box_row_with_right(left_colored: str, right_plain: str):
                # Align right_plain to the far-right inside the box, based on visible length
                left_plain_len = len(strip_ansi(left_colored))
                right_len = len(right_plain)
                # at least 1 space between
                spaces = max(1, inner - left_plain_len - right_len)
                content = left_colored + (" " * spaces) + right_plain
            ...
            def render_job(j: Job):
                ...
                if j.status == "queued":
                    ...
                    status = c("[QUEUED]", DIM)
                    left = f"{jid_col} {id_col} {total_col} {status}"
                    right = f"ENQ {enq}"
                    box_row_with_right(left, right)
                    return
