...
class Job:
    ...
    # result
    error: str = ""
    final_path: str = ""  # full path to file
    ...
    def _render(self):
        ...
        for j in jobs_sorted[:25]:
            if j.status == "downloading":
                ...
                elapsed = fmt_duration((now - j.started_at) if j.started_at else 0)
                eta = j.eta if j.eta else 0
                print(f"{j.job_id} {bar} {pct} {sp} ETA {eta:>4}s ELAP {elapsed}")
            elif j.status == "queued":
                print(f"{j.job_id} [QUEUED] {j.url}")
            elif j.status == "done":
                elapsed = fmt_duration((j.finished_at - j.started_at) if (j.started_at and j.finished_at) else 0)
                saved = j.final_path or "(path unknown)"
                # zobraz kratkou cestu (soubor), ale je to furt "kam ulozeno"
                print(f"{j.job_id} [DONE]  in {elapsed} -> {saved}")
            elif j.status == "fail":
        ...
        def hook(d):
            with self.lock:
                ...
                if st == "downloading":
                    ...
                    # soubor uz je stazeny, muze jeste probehnout merge/postprocess
                    fn = d.get("filename")
                    if fn:
    ...
    def worker_loop(self):
        while not self.stop_event.is_set():
            ...
            try:
                ...
                # kdyz hook nenapsal final_path, dohledame soubor
