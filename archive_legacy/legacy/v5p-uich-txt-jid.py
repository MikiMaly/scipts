...
class Job:
    ...
    enqueued_at: float = 0.0   # time moved into queue (queued)
    added_at: float = 0.0      # time created (pending)
...
class GrabberDaemon:
    def __init__(self, cfg: dict):
        ...
        # NEW: pending urls/jobs (not yet in queue)
        ...
        self.W_JID = 4      # e.g. 0001
        self.W_ID = 17      # job_id / filename base
        self.W_TOTAL = 9
    ...
    def _new_job_id(self, jid: int) -> str:
        # timestamp + jid => unique, readable, no _1
    ...
    def _enqueue_job_into_queue_locked(self, job_id: str):
        # move job into queue (assumes capacity checked under lock)
        j = self.jobs[job_id]
        if j.status != "pending":
            return
        j.status = "queued"
    ...
    def add_url(self, url: str) -> str:
        ...
        with self.lock:
            ...
            # capacity check based on queued items (not pending)
            if self._queued_count_locked() < self.queue_limit:
                self._enqueue_job_into_queue_locked(job_id)
            else:
                # keep as pending and store url (job_id is already created)
    ...
    def _render(self, first: bool = False):
        try:
            ...
            print(
                f"Workers: {self.cfg['max_workers']} | Frags: {fr} | "
            ...
            print("-" * (self.W_JID + 1 + self.W_ID + 1 + self.W_TOTAL + 1 + 64))
            ...
            def render_row(j: Job):
                ...
                if j.status == "queued":
                    ...
                    info = c("ENQ ", DIM) + enq
                    print(f"{jid_col} {id_col} {total_col} {info}")
                    return
...
def main():
    ...
    # CSV autoload at startup => pushes into pending/queue with backpressure
