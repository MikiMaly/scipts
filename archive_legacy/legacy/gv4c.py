    ...
    def _render(self):
        ...
        for j in jobs_sorted[:25]:
            ...
            if j.status == "downloading":
                ...
                eta_color = RED if j.eta_bad else CYAN  # cyan when ok, red when stalling
                eta_part = c("ETA ", DIM) + c(eta_txt, eta_color)
                elap_part = c("ELAP ", DIM) + c(elap_txt, DIM)
                ...
                print(f"{jid} {c('[DONE]', DIM)} in {c(elap_txt, DIM)} -> {c(saved, DIM)}")
        ...
        def hook(d):
            with self.lock:
                ...
                if st == "downloading":
                    ...
                    # ETA stalling heuristic: every ~2 seconds check that ETA drops "enough"
                    now = time.time()
                    if j.last_eta_check_at == 0.0:
                        ...
                        # bad if ETA doesn't drop at least ~half of expected, or increases
