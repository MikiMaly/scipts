...
# - header order: CSV -> Output -> Workers (no blank row)
...
class GrabberDaemon:
    ...
    def _render(self, first: bool = False):
        try:
            ...
            # Header: CSV -> Output -> Workers (no blank row)
            ...
            # ---- compute ELAP column start from the first active row (if exists) ----
            # We measure the *visible* string and find the index where "ELAP " begins.
            # Then queued will place "ENQ ..." starting at that same column.
            elap_col_start = None
            ...
            if active_jobs:
                ...
                # emulate the visible row without colors
            ...
            # fallback if no active job yet
            if elap_col_start is None:
                # reasonable fixed fallback: after "[QUEUED]" area
                elap_col_start = min(inner - 12, 72)
            ...
            def render_job(j: Job):
                ...
                if j.status == "queued":
                    ...
                    # Build LEFT (colored) and decide where ENQ should start (same as ELAP col)
                    left_plain = f"{jid_txt} {id_txt} {total_pad} [QUEUED]"
                    left_colored = f"{jid_col} {id_col} {total_col} {status}"

                    # ENQ should start at elap_col_start
                ...
                if j.status == "done":
                    ...
                    # shorten path to fit
                    prefix_plain = f"{jid_txt} {id_txt} {self._rpad(done_total_txt, self.W_TOTAL)} [DONE] in {elap_txt} -> "
